...
| 变体 | 下载大小 | 适用 |
|---|---|---|
| **cpu** | **194 MB** | 没有 NVIDIA 显卡时选它；任何机器都能跑 |
| **cuda** | **1638 MB** | 有 NVIDIA 显卡时选它，分离速度快很多 |

两者都是**单文件**，直接下载解压即可。

> 早期设计里 CUDA 版是需要 7z 分卷的（含 3.9 GB 的 torch DLL，压缩后仍远超
> GitHub Release 的 2 GB 单文件上限）。模型外置之后压缩后降到 1638 MB，
> 分卷这一步就没必要了。workflow 里仍保留分卷参数（`-v1900m`），
> 但当前体积下不会真的切分 —— 体积再涨上去时也不需要改流程。
